...
In other words, $\overline{S}$ is the hypersurface in the weighted projective space $\mathbb{P}_\mathbb{C}(1,1,1,3)$, which is defined by the equation
$$
\begin{equation*}
...
\end{equation*}
$$
Since deg$(f)=\text{deg}(F)=6$, we can prove that $\mathcal{A}$ is not simultaneously rationalizable by showing that $\overline{S}$ has at most ADE singularities.
...
where `Polynomial([1,0,1])` specifies the coefficients of the polynomial $g(q)$ in the quotient $\mathbb{Q}[q]/g$; in our case $g=1\cdot q^0+0\cdot q^1+1\cdot q^2$.
...
F:=(x+z)*(x-z)*(y+z)*(x+y+z)*(16*x*z+(4*z+y)\cir2);
